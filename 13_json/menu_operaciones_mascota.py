...
def mostrar_mascotas(mascotas):
    if not mascotas:
        ...
        for i, mascota in enumerate(mascotas):
            print(
                f"{i + 1}. {mascota['nombre']} - {mascota['tipo']} - {mascota['edad']} años"
            )
...
def editar_mascota(mascotas):
    ...
    for mascota in mascotas:
        if mascota["nombre"].lower() == nombre_buscar.lower():
            ...
            mascota["nombre"] = input(f"Nuevo nombre (actual: {mascota['nombre']}): ") or \
                                mascota["nombre"]
            mascota["tipo"] = input(f"Nuevo tipo (actual: {mascota['tipo']}): ") or \
                              mascota["tipo"]
...
def eliminar_mascota(mascotas):
    ...
    for i, mascota in enumerate(mascotas):
        if mascota["nombre"].lower() == nombre_buscar.lower():
            confirmacion = input(
                f"¿Estás seguro de eliminar a {mascota['nombre']}? (s/n): ")
